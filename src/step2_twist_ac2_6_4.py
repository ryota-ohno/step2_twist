...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step2_twist.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','A4','phi','E','E_p1','E_p2','E_t1','E_t2','E_t3','E_t4','machine_type','status','file_name'])
        df_E.to_csv(auto_csv_path,index=False)
...
def listen(args):
    ...
    isTest = args.isTest
    opt_param_keys = ['A1','A2','A4']
    fixed_param_keys = ['a','b','phi','theta']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=6:
            ...
            Et1=float(E_list[2]);Et2=float(E_list[3]);Et3=float(E_list[4]);Et4=float(E_list[5]);Ep1=float(E_list[0]);Ep2=float(E_list[1])
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p1':0.,'E_p2':0.,'E_t1':0.,'E_t2':0.,'E_t3':0.,'E_t4':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
    ...
    while True:
        ...
        for A1 in [A1_init_prev-1,A1_init_prev,A1_init_prev+1]:
            for A2 in [A2_init_prev]:
                for A4 in [A4_init_prev-1,A4_init_prev,A4_init_prev+1]:
                    b = np.round(b,1);a = np.round(a,1)
                    df_val_ab = df_val[
                        (df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&
                        (df_val['A1']==A1)&(df_val['A2']==A2)&(df_val['A4']==A4)&(df_val['phi']==phi)&
                        (df_val['status']=='Done')
                                         ]
                    if len(df_val_ab)==0:
                        return False,{'A1':A1,'A2':A2,'A4':A4}
                    heri_list.append([A1,A2,A4]);E_list.append(df_val_ab['E'].values[0])
        A1_init,A2_init,A4_init = heri_list[np.argmin(np.array(E_list))]
