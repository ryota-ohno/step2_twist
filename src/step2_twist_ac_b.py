...
from make_new_ac_6_b import exec_gjf
...
def listen(args):
    ...
    isTest = args.isTest
    opt_param_keys = ['a','b']
    fixed_param_keys = ['A1','A2','phi1','phib','theta']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:
            ...
            Et1=float(E_list[1]);Et3=float(E_list[2]);Ep1=float(E_list[0])
            E = (Et1+Et3)*2+(Ep1)*2
            df_E.loc[idx, ['E_t1','E_t3','E_p1','E','status']] = [Et1,Et3,Ep1,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p1':0.,'E_t1':0.,'E_t3':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
