...
############################汎用関数###########################
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi_r,phi_b,isFF=False):
    ...
    if monomer_name in MONOMER_LIST:
        ...
        #alkyl回転・分子1作成
        xyz_array[C1_index:] = np.matmul((xyz_array[C1_index:]-C1),Rod(n1,phi_b).T) + C1
        ...
        #alkyl回転・分子1作成
        xyz_array[C1_index:] = np.matmul((xyz_array[C1_index:]-C0),Rod(n2,phi_r).T) + C0
...
def make_gaussview_xyz(auto_dir,monomer_name,params_dict,isInterlayer=False):
    ...
    A1 = params_dict['A1']; A2 = params_dict['A2']; A3 = params_dict['theta']; A4 = params_dict['A4']
...
#     monomer_array_i0 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],A1,A2,A3)
    monomer_array_t1 = get_monomer_xyzR(monomer_name,a_/2,b_/2,0,-A4,A2,-A3, phi2)
    monomer_array_t2 = get_monomer_xyzR(monomer_name,a_/2,-b_/2,0,-A4,A2,-A3, phi2)
    monomer_array_t3 = get_monomer_xyzR(monomer_name,-a_/2,-b_/2,0,-A4,A2,-A3, phi2)
    monomer_array_t4 = get_monomer_xyzR(monomer_name,-a_/2,b_/2,0,-A4,A2,-A3, phi2)
...
#     monomer_array_it4 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]+b_/2,c[2],-A1,A2,-A3)
    ...
    file_description = 'A1={}_A2={}_A3={}_A4={}'.format(round(A1),round(A2),round(A3),round(A4))
    ...
    output_path = os.path.join(
        auto_dir,
        'gaussview/{}_A1={}_A2={}_A3={}_A4={}_a={}_b={}.gjf'.format(monomer_name,round(A1),round(A2),round(A3),round(A4),np.round(a_,2),np.round(b_,2))
    )
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,isInterlayer):
    a_ = params_dict['a']; b_ = params_dict['b']; c = np.array([params_dict['cx'],params_dict['cy'],params_dict['cz']])
    A1 = params_dict['A1']; A2 = params_dict['A2']; A3 = params_dict['theta']; A4 = A1
    phi1 = params_dict.get('phi1',0.0); phi2 = -phi1
    phib = params_dict.get('phib',0.0)
    rt = params_dict['Rt']; rp = params_dict['Rp']
    print(phi1, phi2)
    
    monomer_array_i1 = get_monomer_xyzR(monomer_name,0,0,0,A1,A2,A3, phi1,phib)
    monomer_array_i2 = get_monomer_xyzR(monomer_name,0,0,0,-A4,A2,-A3, phi2,phib)
    if a_>b_:
        monomer_array_p1 = get_monomer_xyzR(monomer_name,0,b_,rp,A1,A2,A3, phi1,phib)
        monomer_array_ip1 = get_monomer_xyzR(monomer_name,c[0],c[1]+b_,c[2],A1,A2,A3, phi1,phib)
        monomer_array_ip2 = get_monomer_xyzR(monomer_name,c[0],c[1]-b_,c[2],A1,A2,A3, phi1,phib)
    else:
        monomer_array_p1 = get_monomer_xyzR(monomer_name,a_,0,2*rt-rp,A1,A2,A3, phi1,phib)
        monomer_array_ip1 = get_monomer_xyzR(monomer_name,c[0]+a_,c[1],c[2],A1,A2,A3, phi1,phib)
        monomer_array_ip2 = get_monomer_xyzR(monomer_name,c[0]-a_,c[1],c[2],A1,A2,A3, phi1,phib)
    monomer_array_p2 = get_monomer_xyzR(monomer_name,0,b_,0,-A4,A2,-A3, phi2,phib)
    monomer_array_i0 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],A4,A2,A3, phi1,phib)
    monomer_array_t1 = get_monomer_xyzR(monomer_name,a_/2,b_/2,rt,-A4,A2,-A3, phi2,phib)
    monomer_array_t2 = get_monomer_xyzR(monomer_name,a_/2,-b_/2,rt-rp,-A4,A2,-A3, phi2,phib)
    monomer_array_t3 = get_monomer_xyzR(monomer_name,-a_/2,-b_/2,-rt,-A4,A2,-A3, phi2,phib)
    monomer_array_t4 = get_monomer_xyzR(monomer_name,-a_/2,b_/2,-rt+rp,-A4,A2,-A3, phi2,phib)
    monomer_array_it1 = get_monomer_xyzR(monomer_name,c[0]+a_/2,c[1]+b_/2,c[2],-A4,A2,-A3, phi2,phib)
    monomer_array_it2 = get_monomer_xyzR(monomer_name,c[0]+a_/2,c[1]-b_/2,c[2],-A4,A2,-A3, phi2,phib)
    monomer_array_it3 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]-b_/2,c[2],-A4,A2,-A3, phi2,phib)
    monomer_array_it4 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]+b_/2,c[2],-A4,A2,-A3, phi2,phib)
    ...
    dimer_array_t3 = np.concatenate([monomer_array_i1,monomer_array_t3])
    dimer_array_t4 = np.concatenate([monomer_array_i1,monomer_array_t4])
    ...
    dimer_array_i0 = np.concatenate([monomer_array_i1,monomer_array_i0])
    dimer_array_it1 = np.concatenate([monomer_array_i1,monomer_array_it1])
    dimer_array_it2 = np.concatenate([monomer_array_i1,monomer_array_it2])
    dimer_array_it3 = np.concatenate([monomer_array_i1,monomer_array_it3])
    dimer_array_it4 = np.concatenate([monomer_array_i1,monomer_array_it4])
    dimer_array_ip1 = np.concatenate([monomer_array_i1,monomer_array_ip1])
    dimer_array_ip2 = np.concatenate([monomer_array_i1,monomer_array_ip2])
