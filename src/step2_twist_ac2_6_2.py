...
from make_new_ac_6 import exec_gjf
...
def listen(args):
    ...
    isTest = args.isTest
    fixed_param_keys = ['b','A1','A2','phi','theta']
    opt_param_keys = ['a']
    ...
    maxnum_machine2 = 2#num_nodes/2 if num_nodes%2==0 else (num_nodes+1)/2
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:
            ...
            Et1=float(E_list[1]);Et2=float(E_list[2]);Ep1=float(E_list[0])
            E = 2*(Et1+Et2)+2*(Ep1)
            df_E.loc[idx, ['E_t1','E_t2','E_p1','E','status']] = [Et1,Et2,Ep1,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p1':0.,'E_t1':0.,'E_t2':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    b = init_params_dict['b']; a_init_prev = init_params_dict['a']
    A1 = init_params_dict['A1']; A2 = init_params_dict['A2']; theta = init_params_dict['theta'];phi = init_params_dict['phi']
    
    while True:
        E_list=[];heri_list=[]
        for a in [a_init_prev-0.2,a_init_prev,a_init_prev+0.2]:
            b = np.round(b,1);a = np.round(a,1)
            df_val_ab = df_val[
                (df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&
                (df_val['A1']==A1)&(df_val['A2']==A2)&(df_val['phi']==phi)&
                (df_val['status']=='Done')
                                 ]
            if len(df_val_ab)==0:
                return False,{'a':a}
            heri_list.append(a);E_list.append(df_val_ab['E'].values[0])
        a_init = heri_list[np.argmin(np.array(E_list))]
        if  a_init==a_init_prev:
            return True,{'a':a_init}
        else:
            a_init_prev=a_init
