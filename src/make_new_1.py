...
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    ...
    df_mono=pd.read_csv('~/Working/step2_twist/{}/assets/monomer.csv'.format(monomer_name))
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,isInterlayer):
    ...
    monomer_array_p3 = get_monomer_xyzR(monomer_name,0,b_,0,A1,A2,A3, phi1)
    ...
    dimer_array_p3 = np.concatenate([monomer_array_i,monomer_array_p3])
    dimer_array_p4 = np.concatenate([monomer_array_i,monomer_array_p4])
    ...
    line_list_dimer_p3 = get_xyzR_lines(dimer_array_p3,machine_type,file_description+'_p3')
    line_list_dimer_p4 = get_xyzR_lines(dimer_array_p4,machine_type,file_description+'_p4')
